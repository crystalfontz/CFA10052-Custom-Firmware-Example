...
void Keypad_CheckButtons(void)
{
	...
	//read all buttons
	Read |= (LL_GPIO_IsInputPinSet(KEY_UP_PIN) == 0) ? (1 << KEY_UP) : 0;
	Read |= (LL_GPIO_IsInputPinSet(KEY_DOWN_PIN) == 0) ? (1 << KEY_DOWN) : 0;
	Read |= (LL_GPIO_IsInputPinSet(KEY_LEFT_PIN) == 0) ? (1 << KEY_LEFT) : 0;
	Read |= (LL_GPIO_IsInputPinSet(KEY_RIGHT_PIN) == 0) ? (1 << KEY_RIGHT) : 0;
	Read |= (LL_GPIO_IsInputPinSet(KEY_CANCEL_PIN) == 0) ? (1 << KEY_CANCEL) : 0;
	Read |= (LL_GPIO_IsInputPinSet(KEY_ENTER_PIN) == 0) ? (1 << KEY_ENTER) : 0;

	//debounce
	static uint32_t DebounceTimer = 0;
	static uint8_t LastRead = 0;
	if (!DebounceTimer && (Read != LastRead))
		DebounceTimer = SysTickCount;
	if (DebounceTimer != 0)
	{
		uint32_t Diff = SysTickCount - DebounceTimer;
	...
	for (i = 0; i < BUTTON_COUNT; i++)
	{
		...
		if (State == 1)
		{
			...
			if (ButtonTimer[i] != 0)
			{
				...
				if ((SysTickCount - ButtonTimer[i]) > BUTTON_HOLD_DELAY)
				{
